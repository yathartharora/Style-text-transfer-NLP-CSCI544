...
class OpenAIGPTModel(OpenAIGPTPreTrainedModel):
    ...
    def forward(self, input_ids, position_ids=None, token_type_ids=None):
        ...
        attn_data_list = []
        for block in self.h:
            hidden_states,attn_data = block(hidden_states)
            attn_data_list.append(attn_data)
        output_shape = input_shape + (hidden_states.size(-1),)
        return hidden_states.view(*output_shape), attn_data_list
...
class OpenAIGPTLMHeadModel(OpenAIGPTPreTrainedModel):
    ...
    def forward(self, input_ids, position_ids=None, token_type_ids=None, lm_labels=None):
        hidden_states, attn_data_list = self.transformer(input_ids, position_ids, token_type_ids)
        #lm_logits = self.lm_head(hidden_states)
        #if lm_labels is not None:
            #loss_fct = CrossEntropyLoss(ignore_index=-1)
            #loss = loss_fct(lm_logits.view(-1, lm_logits.size(-1)), lm_labels.view(-1))
            #return loss
        #return lm_logits, attn_data_list
        return hidden_states, attn_data_list
