...
# Get the default cache directory for T5 models
#PYTORCH_PRETRAINED_T5_CACHE = T5Config.from_pretrained('t5-base').cache_dir
...
def main():
    ...
    tokenizer = T5Tokenizer.from_pretrained(args.bert_model, do_lower_case=args.do_lower_case)
    ...
    # Prepare model
    cache_dir = args.cache_dir if args.cache_dir else os.path.join(PYTORCH_PRETRAINED_T5_CACHE, 'distributed_{}'.format(args.local_rank))
    ...
    if args.do_train:
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm_bar):
                ...
                logger.info(segment_ids.shape)
                logger.info(label_ids.shape)
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0:
                    if args.fp16:
                        ...
                        lr_this_step = args.learning_rate * get_linear_schedule_with_warmup(global_step/num_train_optimization_steps, args.warmup_proportion)
    ...
    if args.do_train:
        ...
        config = T5Config(output_config_file)
